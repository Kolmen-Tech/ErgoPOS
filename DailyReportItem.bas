...
Sub Activity_Create(FirstTime As Boolean)
	...
	Activity.LoadLayout("lyDailyReportItem")
...
Sub RefreshGrid
	...
	cursor1 = SQL1.ExecQuery("SELECT * FROM TTMaster INNER JOIN TTDetail ON TTMaster.MasterCode = TTDetail.MasterCode WHERE TTDateInt >= " & (cboTTDateFrom.DateAsLong /1000) & " AND TTDateInt <= " & (cboTTDateTo.DateAsLong /1000) & " AND Status = 1 ORDER BY TTDate")
	For i = 0 To cursor1.RowCount - 1
		...
		strLine1 = cursor1.getString("ItemName") & " x " & cursor1.getString("Qty")
		strLine2 = cursor1.getString("MasterCode") & " | " & cursor1.getString("CustomerName")
		strLine2 = strLine2 & " | " &  strDate '& " " & strTime
		clv1.Add(CreateListItem(strLine1,strLine2,clv1.AsView.Width, 60dip),  dtID)
		strSum = strSum + cursor1.GetString("SubTotalIncTax")
	Next
...
Sub btnExport_Click
	...
	ReportTitle = "Date,Time,DocNo,Customer,CreatedBy,ItemCode,ItemName,Qty,UnitPrice,SubTotal,GSTAmt,SubTotalIncGST"
	...
	For i = 0 To cursor1.RowCount -1
		...
		NewRecord = NewRecord & cursor1.GetString("CreatedBy")  & ","
		NewRecord = NewRecord & cursor1.GetString("ItemCode")  & ","
		NewRecord = NewRecord & cursor1.GetString("ItemName")  & ","
		NewRecord = NewRecord & cursor1.GetString("Qty")  & ","
		NewRecord = NewRecord & cursor1.GetString("UnitPrice")  & ","
		NewRecord = NewRecord & cursor1.GetString("SubTotal")  & ","
		NewRecord = NewRecord & cursor1.GetString("GSTAmt")  & ","
		NewRecord = NewRecord & cursor1.GetString("SubTotalIncTax")
		
		CSVList.Add(NewRecord)
		' cursor1.NextRecord
	Next
   
	File.WriteList(shared, "dailyreportitem.csv", CSVList)
	Msgbox("Export Successfully","EXPORT")
   
	Dim FileName As String = "dailyreportitem.csv"
	...
	'   '(File.Combine(shared,"001/csv"))
	'    inttFile.Initialize(inttFile.ACTION_VIEW ,"file://" & File.Combine(shared,"dailyreportitem.csv"))
...
'	share.sharebinary("file://" & shared & "/dailyreportitem.csv","application/csv","Send File","Are You Sure")
